...
\begin{equation}
  \servouts \equiv \seq{\tup{\serviceid, \hash}} \qquad \gasused \equiv \seq{\tup{\serviceid, \gas}}
\end{equation}
...
\begin{equation}
  \accseq\colon\left\{\begin{aligned}
    ...
    &(g, \mathbf{w}, \mathbf{o}, \mathbf{f}) \!\mapsto\! \begin{cases}
      (0, \mathbf{o}, [], [], []) &
        \when i = 0 \\
      (i + j, \mathbf{o}', \mathbf{t}^* \!\!\concat \mathbf{t}, \mathbf{b}^* \!\!\concat \mathbf{b}, \mathbf{u}^* \!\!\concat \mathbf{u})\!\!\!\! &
        \text{o/w}\!\!\!\!\!\!\!\! \\
    \end{cases} \\
    &\quad\where i = \max(\N_{\len{\mathbf{w}} + 1}): \sum_{w \in \mathbf{w}_{\dots i}}\sum_{r \in w_\mathbf{r}}(r_g) \le g \\
    &\quad\also (\mathbf{o}^*, \mathbf{t}^*, \mathbf{b}^*, \mathbf{u}^*) = \accpar(\mathbf{o}, \mathbf{w}_{\dots i}, \mathbf{f}) \\
    &\quad\also (j, \mathbf{o}', \mathbf{t}, \mathbf{b}, \mathbf{u}) = \accseq(g - \!\!\!\!\sum_{(s, u) \in \mathbf{u}^*}\!\!\!\!u, \mathbf{w}_{i\dots}, \mathbf{o}^*, \{\})
  \end{aligned}\right.
\end{equation}
...
\begin{equation}
  \accpar\colon\left\{\;\begin{aligned}
    ...
    &\ \begin{aligned}
      ...
      \mathbf{u} &= [(s, \accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_u) \mid s \orderedin \mathbf{s}] \\
      \mathbf{b} &= [(s, b) \mid s \orderedin \mathbf{s},\, b = \accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_b,\, b \ne \none] \\
      \mathbf{t} &= [\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{t} \mid s \orderedin \mathbf{s}] \\
      \mathbf{d}' &= \fnprovide((\mathbf{d} \cup \mathbf{n}) \setminus \mathbf{m}, \bigcup_{s \in \mathbf{s}} \accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{p}) \\
      &(\mathbf{d}, \mathbf{i}, \mathbf{q}, m, \mathbf{z}) = \mathbf{o} \\
      (m', \mathbf{a}^*, v^*, \mathbf{z}') &= (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, m)_\mathbf{o})_{(m, \mathbf{a}, v, \mathbf{z})} \\
      \forall c \in \coreindex : \mathbf{a}'_c &= ((\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, \mathbf{a}^*_c)_\mathbf{o})_\mathbf{a})_c \\
      v' &= (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, v^*)_\mathbf{o})_v \\
      \mathbf{i}' &= (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, v)_\mathbf{o})_\mathbf{i} \\
      \forall c \in \coreindex : \mathbf{q}'_c &= (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, \mathbf{a}_c)_\mathbf{o})_\mathbf{q} \\
      \mathbf{n} &= \bigcup_{s \in \mathbf{s}}(\{ (\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{o})_\mathbf{d} \setminus \Fkeys{\mathbf{d} \setminus \{s\}} \}) \\
      \mathbf{m} &= \bigcup_{s \in \mathbf{s}}(\Fkeys{\mathbf{d}} \setminus \Fkeys{(\accone(\mathbf{o}, \mathbf{w}, \mathbf{f}, s)_\mathbf{o})_\mathbf{d}})
    \end{aligned}
  \end{aligned}\right.
\end{equation}
...
\end{equation}
\begin{align}
    &\accone \colon \left\{\;\begin{aligned}
    &\begin{aligned}
      \tup{\begin{aligned}
...
\begin{align}
  ...
  &(n, \mathbf{o}', \mathbf{t}, \lastaccout', \mathbf{u}) \equiv
    \accseq(g, \mathbf{W}^*, \mathbf{o}, \alwaysaccers) \\
